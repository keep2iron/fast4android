...
class GradientBackgroundView : View {
  ...
  constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(
    ...
    attrs?.let {
      currentColor = array.getColor(
        R.styleable.GradientBackgroundView_gbv_default_color,
        ContextCompat.getColor(context, R.color.white)
      )
      duration =
        array.getInteger(R.styleable.GradientBackgroundView_gbv_duration, duration.toInt()).toLong()
    }
  ...
  fun animatorNextColor(weight: Float, @ColorRes color: Int) {
    ...
    this.radius = Math.sqrt(
      Math.pow((maxWeight * measuredWidth).toDouble(), 2.0)
          + Math.pow(measuredHeight.toDouble(), 2.0)
    ).toFloat()
