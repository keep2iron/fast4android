...
public class NineGridImageView extends ViewGroup {
  ...
  public static abstract class AbstractAdapter {
    ...
    View onCreateView(@NonNull ViewGroup viewParent, @NonNull Context context);

    public abstract void onBindView(@NonNull View view, int position);
  ...
  public NineGridImageView(Context context, AttributeSet attrs) {
    ...
    this.mSingleImgSize =
        typedArray.getDimensionPixelSize(R.styleable.NineGridImageView_singleImgSize, -1);
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (mAdapter != null && mAdapter.getItemSize() > 0) {
      if (mAdapter.getItemSize() == 1 && mSingleImgSize != -1) {
        mGridSize = mSingleImgSize > totalWidth ? totalWidth : mSingleImgSize;
      } else {
        //                mList.get(0).setScaleType(ImageView.ScaleType.CENTER_CROP);
  ...
  private void layoutChildrenView() {
    if (mAdapter == null || mAdapter.getItemSize() == 0) return;
    int showChildrenCount = getNeedShowCount(mAdapter.getItemSize());
  ...
  private void layoutForNoSpanChildrenView(int childrenCount) {
    ...
    for (int i = 0; i < childrenCount; i++) {
      View childrenView = (View) getChildAt(i);
      if (childrenView != null) {
        ...
        if (mAdapter != null) {
          mAdapter.onBindView(childrenView, i);
        }
  ...
  public void setAdapter(AbstractAdapter adapter) {
    setImagesData(adapter, NOSPAN);
  }

  /**
   * 设置图片数据
   *
   * @param adapter 图片数据集合
   * @param spanType 跨行跨列排版类型
   */
  public void setImagesData(AbstractAdapter adapter, int spanType) {
    if (adapter == null) {
    ...
    int newShowCount = getNeedShowCount(adapter.getItemSize());

    int[] gridParam = calculateGridParam(newShowCount, mShowStyle);
    mRowCount = gridParam[0];
    mColumnCount = gridParam[1];
    mAdapter = adapter;
    int oldShowCount = getNeedShowCount(getChildCount());
  ...
  private View getView(final int position) {
    if (mAdapter != null) {
      View view = mAdapter.onCreateView(this, getContext());
