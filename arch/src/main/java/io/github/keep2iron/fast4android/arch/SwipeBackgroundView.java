  ...
  public void bind(Activity activity, Activity swipeActivity, boolean restoreForSubWindow) {
    ...
    if (orientation != getResources().getConfiguration().orientation) {
      ...
      if (requestedOrientation == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE ||
        ...
        mDoRotate = true;
      } else if (swipeActivity instanceof AbstractActivity){
        swipeActivity.getWindow().getDecorView().setBackgroundColor(0);
        ((AbstractActivity) swipeActivity).convertToTranslucentCauseOrientationChanged();
        invalidate();
        return;
      }
    }
    ...
    try {
      ...
      Field windowManagerGlobalField =
          activity.getWindowManager().getClass().getDeclaredField("mGlobal");
      ...
      if (windowManagerGlobal != null) {
        ...
        List<WindowManager.LayoutParams> params =
            (List<WindowManager.LayoutParams>) paramsField.get(windowManagerGlobal);
