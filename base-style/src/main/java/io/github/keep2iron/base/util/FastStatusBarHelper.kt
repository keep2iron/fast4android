...
object FastStatusBarHelper {
  ...
  @StatusBarType private var mStatusBarType =
  ...
  fun setStatusBarLightMode(activity: Activity?): Boolean {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
      if (isMIUICustomSTATUS_BARLightModeImpl && MIUISetStatusBarLightMode(
          activity.window,
          true
        )
      ) {
  ...
  private fun setStatusBarLightMode(activity: Activity, @StatusBarType type: Int): Boolean {
    return when (type) {
      STATUS_BAR_TYPE_MIUI -> MIUISetStatusBarLightMode(
        activity.window,
        true
      )
      STATUS_BAR_TYPE_FLYME -> FlymeSetStatusBarLightMode(
        activity.window,
        true
      )
      STATUS_BAR_TYPE_ANDROID6 -> Android6SetStatusBarLightMode(
        activity.window,
        true
      )
      else -> false
  ...
  private fun initStatusBarHeight(context: Context) {
    ...
    if (FastDeviceHelper.isTablet(context) && sStatusBarHeight > dp2px(
        context, STATUS_BAR_DEFAULT_HEIGHT_DP
      )
    ) {
      ...
      if (sStatusBarHeight <= 0) {
        sStatusBarHeight = if (sVirtualDensity == -1f) {
          dp2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP)
        } else {
