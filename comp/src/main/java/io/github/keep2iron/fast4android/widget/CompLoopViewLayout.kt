...
import io.github.keep2iron.fast4android.adapter.LooperWrapperPagerAdapter
...
class CompLoopViewLayout : FrameLayout {
  ...
  private var indicatorWidth: Int = dp2px(10f)
  private var indicatorHeight: Int = dp2px(10f)
  private var indicatorMargin: Int = dp2px(10f)
  private var flNoDataContainer: FrameLayout
  private var lastPosition: Int = 0

  private var onPageChangedListener: androidx.viewpager.widget.ViewPager.OnPageChangeListener? = null
  ...
  constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(
    ...
    indicatorSelectResId = array.getResourceId(
      R.styleable.CompLoopViewLayout_comp_loopLayout_indicatorDrawable_selected,
      R.drawable.comp_shape_gray_radius
    )
    indicatorUnselectResId = array.getResourceId(
      R.styleable.CompLoopViewLayout_comp_loopLayout_indicatorDrawable_unselected,
      R.drawable.comp_shape_white_radius
    )
    indicatorWidth = array.getDimensionPixelSize(
      R.styleable.CompLoopViewLayout_comp_loopLayout_indicatorDrawable_width,
      indicatorWidth
    )
    indicatorHeight = array.getDimensionPixelSize(
      R.styleable.CompLoopViewLayout_comp_loopLayout_indicatorDrawable_height,
      indicatorHeight
    )
    indicatorMargin = array.getDimensionPixelSize(
      R.styleable.CompLoopViewLayout_comp_loopLayout_indicatorDrawable_margin,
      indicatorMargin
    )
    array.recycle()
  ...
  private fun initOnPageChangedListener() {
    viewPager.addOnPageChangeListener(object : androidx.viewpager.widget.ViewPager.OnPageChangeListener {
      ...
      override fun onPageScrolled(position: Int, positionOffset: Float, positionOffsetPixels: Int) {
        onPageChangedListener?.onPageScrolled(
      ...
      override fun onPageSelected(position: Int) {
        ...
        (indicators.getChildAt(toRealPosition(currentPosition)) as ImageView).setImageResource(
          indicatorUnselectResId
        )
   ...
   * @param pool 使用
   */
  fun <T : RecyclerViewHolder> setAdapter(
    recycleAdapter: androidx.recyclerview.widget.RecyclerView.Adapter<T>,
    pool: androidx.recyclerview.widget.RecyclerView.RecycledViewPool = androidx.recyclerview.widget.RecyclerView.RecycledViewPool()
  ) {
    viewPager.removeAllViews()
    this.adapter = LooperWrapperPagerAdapter(recycleAdapter, pool)
    recycleAdapter.registerAdapterDataObserver(object : androidx.recyclerview.widget.RecyclerView.AdapterDataObserver() {
      override fun onChanged() {
        currentPosition = DEFAULT_POSITION
        createIndicator()
        this@CompLoopViewLayout.adapter.notifyDataSetChanged()
        flNoDataContainer.visibility =
          if (recycleAdapter.itemCount == 0) View.VISIBLE else View.GONE
      }
  ...
  fun getRecyclerViewAdapter(): androidx.recyclerview.widget.RecyclerView.Adapter<out androidx.recyclerview.widget.RecyclerView.ViewHolder> {
