...
object FastDisplayHelper {
  ...
  fun getScreenHeight(context: Context): Int {
    ...
    if (FastDeviceHelper.isXiaomi && xiaomiNavigationGestureEnabled(
        context
      )
    ) {
  ...
  fun getUsefulScreenWidth(activity: Activity): Int {
    return getUsefulScreenWidth(
      activity,
      FastNotchHelper.hasNotch(activity)
    )
  }
  ...
  fun getUsefulScreenWidth(context: Context, hasNotch: Boolean): Int {
    ...
    if (!hasNotch) {
      if (isLandscape && FastDeviceHelper.isEssentialPhone
        ...
        result -= 2 * FastStatusBarHelper.getStatusBarHeight(
          context
        )
      }
      return result
    ...
    if (isLandscape) {
      ...
      if (FastDeviceHelper.isHuawei && !huaweiIsNotchSetToShowInSetting(
          context
        )
      ) {
        result -= FastNotchHelper.getNotchSizeInHuawei(
          context
        )[1]
      }
  ...
  fun xiaomiNavigationGestureEnabled(context: Context): Boolean {
    val `val` = if (VERSION.SDK_INT >= VERSION_CODES.JELLY_BEAN_MR1) {
