...
object FastNotchHelper {
  ...
  private val TAG = "FastNotchHelper"
  ...
  fun hasNotchInXiaomi(context: Context): Boolean {
    try {
      ...
      val getMethod =
        spClass.getDeclaredMethod("getInt", String::class.java, Int::class.javaPrimitiveType)
      getMethod.isAccessible = true
      val hasNotch = getMethod.invoke(null,
        MIUI_NOTCH, 0) as Int
      return hasNotch == 1
    } catch (e: Exception) {
      e.printStackTrace()
  ...
  private fun get3rdSafeInsetRect(context: Context): Rect? {
    ...
    if (FastDeviceHelper.isHuawei) {
      val isHuaweiNotchSetToShow =
        FastDisplayHelper.huaweiIsNotchSetToShowInSetting(
          context
        )
  ...
  private fun getRectInfoRotation0(context: Context): Rect {
    ...
    if (FastDeviceHelper.isVivo) {
      ...
      rect.top =
        FastStatusBarHelper.getStatusBarHeight(context)
      rect.bottom = 0
      ...
      rect.top =
        getNotchHeightInXiaomi(context)
      rect.bottom = 0
    }
    return rect
  }
  ...
  private fun getRectInfoRotation90(context: Context): Rect {
    ...
    if (FastDeviceHelper.isVivo) {
      ...
      if (sHuaweiIsNotchSetToShow!!) {
        rect.left = getNotchSizeInHuawei(
          context
        )[1]
      } else {
        rect.left = 0
  ...
  private fun getRectInfoRotation270(context: Context): Rect {
    ...
    if (FastDeviceHelper.isVivo) {
      ...
      if (sHuaweiIsNotchSetToShow!!) {
        rect.right = getNotchSizeInHuawei(
          context
        )[1]
      } else {
        rect.right = 0
  ...
  fun getNotchHeightInXiaomi(context: Context): Int {
    ...
    return if (resourceId > 0) {
      context.resources.getDimensionPixelSize(resourceId)
    } else FastDisplayHelper.getStatusBarHeight(context)
  }

  fun getNotchWidthInVivo(context: Context): Int {
    return FastDisplayHelper.dp2px(context, 100)
  }

  fun getNotchHeightInVivo(context: Context): Int {
    return FastDisplayHelper.dp2px(context, 27)
  }
  ...
  private fun getScreenRotation(context: Context): Int {
    val w =
      context.getSystemService(Context.WINDOW_SERVICE) as WindowManager ?: return Surface.ROTATION_0
  ...
  fun needFixLandscapeNotchAreaFitSystemWindow(view: View): Boolean {
    return FastDeviceHelper.isXiaomi && hasNotch(
      view
    )
  }
