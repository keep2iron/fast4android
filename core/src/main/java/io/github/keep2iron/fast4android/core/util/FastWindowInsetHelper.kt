...
class FastWindowInsetHelper(viewGroup: ViewGroup, windowInsetLayout: IWindowInsetLayout) {
  ...
  fun defaultApplySystemWindowInsets19(viewGroup: ViewGroup, insets: Rect): Boolean {
    ...
    if (insets.bottom >= KEYBOARD_HEIGHT_BOUNDARY) {
      ...
      viewGroup.setTag(R.id.fast_window_inset_keyboard_area_consumer,
        KEYBOARD_CONSUMER
      )
      insets.bottom = 0
    } else {
  ...
  private fun dispatchNotchInsetChange(view: View) {
    if (view is io.github.keep2iron.fast4android.core.widget.INotchInsetConsumer) {
      val stop = (view as io.github.keep2iron.fast4android.core.widget.INotchInsetConsumer).notifyInsetMaybeChanged()
      if (stop) {
        return
      }
    }
