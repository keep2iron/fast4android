...
object FastDisplayHelper {
  ...
  fun getScreenHeight(context: Context): Int {
    ...
    if (FastDeviceHelper.isXiaomi && xiaomiNavigationGestureEnabled(context)) {
  ...
  fun getUsefulScreenWidth(activity: Activity): Int {
    return getUsefulScreenWidth(activity, FastNotchHelper.hasNotch(activity))
  }

  fun getUsefulScreenWidth(view: View): Int {
    return getUsefulScreenWidth(view.context, FastNotchHelper.hasNotch(view))
  }
  ...
  fun getUsefulScreenWidth(context: Context, hasNotch: Boolean): Int {
    ...
    if (isLandscape) {
      ...
      if (FastDeviceHelper.isHuawei && !FastDisplayHelper.huaweiIsNotchSetToShowInSetting(context)) {
        result -= FastNotchHelper.getNotchSizeInHuawei(context)[1]
      }
  ...
  fun getUsefulScreenHeight(activity: Activity): Int {
    return getUsefulScreenHeight(activity, FastNotchHelper.hasNotch(activity))
  }

  fun getUsefulScreenHeight(view: View): Int {
    return getUsefulScreenHeight(view.context, FastNotchHelper.hasNotch(view))
  }
  ...
  private fun getUsefulScreenHeight(context: Context, hasNotch: Boolean): Int {
    ...
    if (!hasNotch) {
      if (isPortrait && FastDeviceHelper.isEssentialPhone
        && Build.VERSION.SDK_INT < Build.VERSION_CODES.O
      ) {
        // https://arstechnica.com/gadgets/2017/09/essential-phone-review-impressive-for-a-new-company-but-not-competitive/
        // 这里说挖孔屏是状态栏高度的两倍
  ...
  fun hasNavigationBar(context: Context): Boolean {
    ...
    return if (FastDeviceHelper.isVivo) {
