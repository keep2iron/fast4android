...
object FastStatusBarHelper {
  ...
  @StatusBarType private var mStatusBarType = STATUS_BAR_TYPE_DEFAULT
   ...
   */
  private val isMIUICustomSTATUS_BARLightModeImpl: Boolean
    get() = if (FastDeviceHelper.isMIUIV9 && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
      true
    } else FastDeviceHelper.isMIUIV5 || FastDeviceHelper.isMIUIV6 ||
        FastDeviceHelper.isMIUIV7 || FastDeviceHelper.isMIUIV8
  ...
  private fun supportTranslucent(): Boolean {
    return (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT
        ...
        && !(FastDeviceHelper.isEssentialPhone && Build.VERSION.SDK_INT < 26))
  }
  ...
  fun translucent(window: Window, @ColorInt colorOn5x: Int) {
    ...
    if (FastDeviceHelper.isMeizu || FastDeviceHelper.isMIUI && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
      window.setFlags(
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && supportTransclentStatusBar6()) {
        ...
        // 采取setSTATUS_BARColor的方式，部分机型不支持，那就纯黑了，保证状态栏图标可见
        window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS)
        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)
  ...
  fun setStatusBarLightMode(activity: Activity?): Boolean {
    ...
    if (FastDeviceHelper.isZTKC2016) {
      return false
    }
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
      if (isMIUICustomSTATUS_BARLightModeImpl && MIUISetStatusBarLightMode(activity.window, true)) {
   ...
   *
   * @param activity 需要被处理的 Activity
   * @param type     STATUS_BAR 类型，对应不同的系统
   */
  private fun setStatusBarLightMode(activity: Activity, @StatusBarType type: Int): Boolean {
    return when (type) {
      STATUS_BAR_TYPE_MIUI -> MIUISetStatusBarLightMode(activity.window, true)
      STATUS_BAR_TYPE_FLYME -> FlymeSetStatusBarLightMode(activity.window, true)
      STATUS_BAR_TYPE_ANDROID6 -> Android6SetStatusBarLightMode(activity.window, true)
      else -> false
    }
  }
  ...
  fun setStatusBarDarkMode(activity: Activity?): Boolean {
    ...
    return when (mStatusBarType) {
      STATUS_BAR_TYPE_MIUI -> MIUISetStatusBarLightMode(activity.window, false)
      STATUS_BAR_TYPE_FLYME -> FlymeSetStatusBarLightMode(activity.window, false)
      STATUS_BAR_TYPE_ANDROID6 -> Android6SetStatusBarLightMode(activity.window, false)
      else -> true
    }
  }
  ...
  private fun changeStatusBarModeRetainFlag(window: Window, out: Int): Int {
    var varOut = out
    varOut = retainSystemUiFlag(window, varOut, View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN)
    varOut = retainSystemUiFlag(window, varOut, View.SYSTEM_UI_FLAG_FULLSCREEN)
    varOut = retainSystemUiFlag(window, varOut, View.SYSTEM_UI_FLAG_HIDE_NAVIGATION)
    varOut = retainSystemUiFlag(window, varOut, View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY)
    varOut = retainSystemUiFlag(window, varOut, View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN)
    varOut = retainSystemUiFlag(window, varOut, View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION)
    return varOut
  }
  ...
  /**
   * 检测 Android 6.0 是否可以启用 window.setSTATUS_BARColor(Color.TRANSPARENT)。
   */
  fun supportTransclentStatusBar6(): Boolean {
    return !(FastDeviceHelper.isZUKZ1 || FastDeviceHelper.isZTKC2016)
  }
  ...
  private fun initStatusBarHeight(context: Context) {
    ...
    if (FastDeviceHelper.isTablet(context) && sStatusBarHeight > dp2px(
      ...
      if (sStatusBarHeight <= 0) {
        sStatusBarHeight = if (sVirtualDensity == -1f) {
          dp2px(STATUS_BAR_DEFAULT_HEIGHT_DP)
        } else {
