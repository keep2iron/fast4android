...
import io.github.keep2iron.fast4android.widget.INotchInsetConsumer
import io.github.keep2iron.fast4android.widget.IWindowInsetLayout
...
class FastWindowInsetHelper(viewGroup: ViewGroup, windowInsetLayout: IWindowInsetLayout) {
  ...
  companion object {
    private val KEYBOARD_HEIGHT_BOUNDARY: Int = dp2px(100)
  ...
  fun defaultApplySystemWindowInsetsCompat(
    ...
    for (i in 0 until viewGroup.childCount) {
      ...
      if (FastNotchHelper.needFixLandscapeNotchAreaFitSystemWindow(viewGroup) && viewGroup.resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE) {
        insetLeft = Math.max(insetLeft, FastNotchHelper.getSafeInsetLeft(viewGroup))
        insetRight = Math.max(insetRight, FastNotchHelper.getSafeInsetRight(viewGroup))
      }
  ...
  private fun dispatchNotchInsetChange(view: View) {
    if (view is INotchInsetConsumer) {
      val stop = (view as INotchInsetConsumer).notifyInsetMaybeChanged()
      if (stop) {
        return
      }
    }
