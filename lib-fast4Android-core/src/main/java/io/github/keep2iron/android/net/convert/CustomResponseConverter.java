...
import io.github.keep2iron.android.net.exception.StatusErrorException;
...
class CustomResponseConverter<T> implements Converter<ResponseBody, T> {
    ...
    <C extends CustomConvertFactory.IResponseStatus> CustomResponseConverter(Gson gson, Type type, Class<C> clazz) {
        this.gson = gson;
        this.type = type;
        this.clazz = clazz;
    }
    ...
    private T convertByStatusTest(String body) throws StatusErrorException {
        CustomConvertFactory.IResponseStatus status = gson.fromJson(body, clazz);
